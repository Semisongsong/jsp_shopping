...
<script>
		function dupcheck() {
			var uid = document.product_form.num.value
			if (uid == "") {
				alert("아이디를 입력하여 주십시오.");
				document.idid.id.value.focus();
				return;
			} else {
				var url = "dupchk?userid=" + uid;
...
<script>
 function mySubmit(index){
	 if(index==1) {
		 document.product_form.action='<%=request.getContextPath()%>/ShoppingServlet?command=updatecart';
 	...
 	if(index==2) {
 		 if (confirm("주문하시겠습니까?")){
 			document.product_form.action='<%=request.getContextPath()%>/ShoppingServlet?command=order'; 
 			document.product_form.submit(); 
    ...
    <div class="cart-box-main">
        <div class="container">
    <form class="product_form" name="product_form"  method="post">
            <div class="row">
                <div class="col-lg-12">
                    <div class="table-main table-responsive">
            			...
            			<input type="hidden" name="num" value="${list.num}">
            			   <input type="hidden" name="name" value="${list.name}">
                                ...
                                <tr>
                               <td class>
                                <input type="checkbox" name="chk[]" value="${list.num}" checked="${list.num}" id="abc">
                            </td>
                                    <td class="thumbnail-img">
                                        <a href="<%=request.getContextPath()%>/freshshop-master/#">
									<img class="img-fluid" src="./freshshop-master/upload/${list.image}" alt="" />
								</a>
                                    </td>
                                    <td class="name-pr">
                                        <a href="<%=request.getContextPath()%>/freshshop-master/#">
                                    ...
                                    <td class="quantity-box"><input class="form-control" value="${list.amount}" min="1" max="20" type="number" name="amount" ></td>
                                    <td class="total-pr">
                                        <p>₩  ${list.price*list.amount}</p>
                                    </td>
                                 
                                    
                                   <!--  <td class="quantity-box"><input type="number" size="4" name="selAmount" value="${list.amount}" min="1" step="1" class="c-input-text qty text"></td>
                                    <td class="total-pr">
                                        <p>$ ${list.amount * list.price }</p>
                                    </td>--> 
                                    
                                    <td class="add-pr"><input type="button" class="btn hvr-hover"
										 onClick="mySubmit(1);" value="mod">
										</td>
                                    <td class="remove-pr">
                                        <a href="/123/ShoppingServlet?command=delcart&num=${list.num}">
            ...
            <!--  <div class="row my-5">
                ...
                <div class="col-12 d-flex shopping-box"> <input type="button" class="ml-auto btn hvr-hover"
										 onClick="mySubmit(2);" value="Checkout"> </div>
